...
import {
  queryConversationIdsFromKnowledgeStore,
  queryConversationMatchesFromKnowledgeStore,
  queryConversationsFromKnowledgeStore,
  queryDashboardStatsFromKnowledgeStore,
  queryTopicsWithCountsFromKnowledgeStore
} from "./knowledgeQueryStore"
...
import { getStorageEngineState } from "./storageEngineState"
          ...
          toolPlan: Array.isArray(meta.plan.toolPlan)
            ? meta.plan.toolPlan.filter(
                (
                  ...
                  typeof meta.plan
                >["toolPlan"][number] => typeof toolName === "string"
              )
            : undefined
        }
      : undefined
...
function normalizeConversationFilterPlatforms(
...
async function listConversationsFromDexie(
  filters?: ConversationFilters
): Promise<Conversation[]> {
  const platforms = normalizeConversationFilterPlatforms(filters)
  let results = await db.conversations.toArray()

  if (platforms.length > 0) {
    const platformSet = new Set(platforms)
    results = results.filter((record) => platformSet.has(record.platform))
  }

  if (filters?.includeTrash === false) {
    results = results.filter((record) => !record.is_trash)
  }

  if (filters?.includeArchived === false) {
    results = results.filter((record) => !record.is_archived)
  }
  ...
  if (normalizedSearch) {
    results = results.filter(
      (record) =>
        ...
        record.snippet.toLowerCase().includes(normalizedSearch)
    )
  }
  ...
  if (rangeStart !== null && rangeEnd !== null && rangeEnd >= rangeStart) {
    results = results.filter((record) => {
      const originAt = getConversationOriginAt(toConversation(record))
  ...
  return results
    .sort(
      (left, right) =>
        getConversationOriginAt(toConversation(right)) -
        getConversationOriginAt(toConversation(left))
    )
    .map(toConversation)
}

function buildTopicTree(topicRows: Topic[]): Topic[] {
...
export async function listConversations(
  ...
  const sqliteResult = await queryConversationsFromKnowledgeStore(filters)
  if (sqliteResult !== null) {
    return sqliteResult
  }

  return listConversationsFromDexie(filters)
}
...
export async function getTopics(): Promise<Topic[]> {
  const sqliteResult = await queryTopicsWithCountsFromKnowledgeStore()
  if (sqliteResult !== null) {
    return buildTopicTree(sqliteResult)
  }
  ...
  return buildTopicTree(
    topicRecords
      .filter(
        (record): record is TopicRecord & { id: number } =>
...
export async function searchConversationIdsByText(
  ...
  const sqliteResult = await queryConversationIdsFromKnowledgeStore(query)
  if (sqliteResult !== null) {
    return sqliteResult
  }
...
export async function searchConversationMatchesByText(
  ...
  const sqliteResult = await queryConversationMatchesFromKnowledgeStore(params)
  if (sqliteResult !== null) {
    return sqliteResult
  }
...
export async function getDataOverview(): Promise<DataOverviewSnapshot> {
  ...
  return {
    ...
    storageEngine: {
      activeEngine: storageEngine.activeEngine,
      migrationState: storageEngine.migrationState,
      snapshotWatermark: storageEngine.snapshotWatermark,
      appliedWatermark: storageEngine.appliedWatermark,
      lastError: storageEngine.lastError
    }
  }
}
...
export async function getDashboardStats(): Promise<DashboardStats> {
  const sqliteResult = await queryDashboardStatsFromKnowledgeStore()
  if (sqliteResult !== null) {
    return sqliteResult
  }
