...
* **捕获引擎**：Parser / Observer / Gate 三层架构。Observer 层使用 MutationObserver 监听 DOM 变化，Parser 层根据平台特性提取对话内容（每个平台定义多套主备 Selector 策略，运行时动态检测有效路径），Gate 层控制是否触发解析流程。存储层使用 debounce 机制与增量更新策略，大幅减少数据库写操作频次。
...
解压安装包到本地任意位置，确保能直接看到 manifest.json 文件。注意安装完成后请勿删除或移动该文件夹。
在 Chrome 浏览器地址栏输入 chrome://extensions/ 进入扩展管理页，开启右上角的"开发者模式"开关。
点击左上角的"加载已解压的扩展程序"按钮，选择解压后的文件夹。
...
阶段核心动作具体路径 / 参数构建与加载选择扩展目录开发模式：frontend/build/chrome-mv3-dev生产模式：frontend/build/chrome-mv3-prod获取 API在官网获取令牌注册登录 ➔ 个人中心 ➔ 访问令牌 ➔ 复制 (sdk_ / ms_ 开头)插件内配置填入设置信息进入设置 ➔ 填入 API Key ➔ 填入 Model ID ➔ 点击 Test日常使用侧边栏捕获与摘要对话详情页 ➔ 点击"生成摘要"（结果自动本地缓存）知识库探索打开 Web Dashboard点击扩展图标 ➔ 进入 Knowledge 入口 ➔ 在 Library / Explore / Network / Notes 间导航
...
浏览与搜索
点击浏览器工具栏上的心迹图标，或通过侧边栏快捷方式，可以打开心迹面板。默认页面是时间轴视图，显示所有捕获的对话按时间倒序排列。将鼠标悬停在卡片上，会展开显示对话的前100字预览，以及编辑、打开原页面、删除等快捷操作。点击卡片进入详情视图，显示完整的对话历史。
在顶部搜索框中输入关键词，可以实时过滤对话列表。搜索支持标题匹配与内容全文匹配，结果高亮显示匹配文字，并可按平台标签筛选。如果需要跨越关键词边界的语义检索——例如搜索"缓存策略"能找到讨论 Redis 的对话——请前往 Web Dashboard 的 Explore Tab 使用向量检索模式。
查看详情
在对话详情页中，消息按时间正序排列，用户消息与 AI 消息通过不同背景色区分。长消息支持智能折叠，超过500字符的内容自动收起，显示"展开全文"按钮。将鼠标悬停在任意消息上，右上角会出现复制按钮。如果消息中包含代码块，会自动应用等宽字体与语法高亮。
...
心迹提供两种 API 调用模式，可在侧边栏右下角的设置 (Settings) → Model Access 面板中切换：
...
生成 Thread Summary
在配置好 ModelScope API 后，对话详情页会显示"生成摘要"按钮。点击后，系统经由 Prompt-as-Code 管理的提示词模板生成结构化分析，包含五个部分：核心问题（用一句话概括对话意图）、思维演进（关键转折的过程）、核心洞见（最有价值的认知收获）、悬而未决的话题（未深入探讨的问题）、可行的下一步（后续思考或行动建议）。
...
使用知识管理平台
点击扩展图标后进入 Knowledge 入口，即可打开 Web Dashboard。平台包含四个 Tab，建议按以下方式理解各自的使用场景：
...
Explore 是你向自己的历史思考提问的地方。在输入框中用自然语言描述你想找的内容，系统在本地向量库中检索相关对话，将其作为上下文生成有引用来源的回答。每条回答底部的来源卡片可点击跳转回原始对话，确保系统的每个结论都有据可查。当本地相关度低于阈值时，系统会动态切换至大模型世界知识进行兜底，并在回答中明确标注。
Network 以可交互的力导向图呈现你的思维网络。第一次打开时可能需要片刻时间让后台完成相似度矩阵的计算。拖动节点可以手动调整布局，Hover 显示对话标题，点击节点高亮其关联子网并在侧边栏显示跳转入口。节点大小映射讨论深度，边的粗细反映语义相似度，颜色对应平台。
Notes 是你沉淀个人洞察的空间。点击任意空白区域进入编辑态，支持标准 Markdown 语法，失焦后自动保存并切换回阅读态。笔记可以关联特定对话（从 Library 的 Reader View 导入），也可以作为独立条目存在。被手动策展进 Notes 的知识，在 Explore 的检索权重中高于普通对话。
生成周度洞察
...
数据管理
在侧边栏导航到 Data 页面，可以全面掌控自己的本地数据。页面分为三个组：Overview 显示存储用量、对话总数、摘要记录数等聚合统计；Operations 提供导出与清理操作；Roadmap 展示近期能力规划（当前为只读视图，Dashboard 完整版即将推出）。
存储保护机制：数据量达到 900MB 时进行软警告，达到 1GB 时触发硬限制暂停新数据写入。
导出支持三种格式：JSON（标准无损备份，包含完整结构化数据，适合未来跨设备迁移）、TXT（人类可读档案，含元数据标头与时间顺序排版）、Markdown（适合直接导入个人知识库软件）。
清理操作分为两个独立动作：对话历史与 Insights 缓存可以分别清除，防止误操作波及不想删除的数据。执行危险操作前需精确输入 DELETE 指令确认。你的 API 密钥与模型配置在清理后完好保留。
Settings 信息密度与 Support 入口
为保证 Settings 的可读性与操作效率，v1.4 起采用以下固定规则：分组结构固定为 Personalisation / System / Support；Support 固定三项平铺行（非 accordion）：Docs & Help、Send Feedback（行内展开）、What's New；Settings 展开区仅保留操作指令、当前状态与即时警告三类文案，详细背景解释统一下沉到 README。
这意味着 Settings 页面负责"完成动作"，README 负责"解释原理"。
---

## 🔬 从记录工具到智能知识中台：一次有自我意识的迭代

> 我们试图回答一个诚实的问题：心迹最初的产品理念，是否在工程上得到了真正的兑现？

### 场景价值的再锚定

心迹早期版本的产品叙事有一个内在张力：它的哲学层面讲述的是"思维主权"与"意义的关系性"，但功能层面交付的，不过是"把 AI 对话存到本地"。我们在理念一节中写道，"意义是关系性的而非本质性的"——单个对话的重要性来自于它与其他对话的关联。然而在相当长的时间里，产品没有任何机制去发现、建立或呈现这种关联。用户存储了数十乃至数百个对话，但每一条都是孤立的信息孤岛。这不是在帮助用户找回叙事，这只是在换一个地方堆放碎片。

本轮迭代的核心驱动力，正是这个认知上的不诚实。我们重新定义了场景边界：心迹不应当是一个"记录你和 AI 说过什么"的存档系统，而应当是一个"帮助你理解自己在想什么、关注什么、以及这些思考如何随时间演化"的个人知识中台。Library、Explore、Network、Notes 四个模块共同构成了完整的知识生命周期闭环：捕获 → 分类 → 关联发现 → 沉淀。用户不再需要记得"那个关于 React 性能的讨论发生在哪一天"——他可以直接问，系统在本地知识库中检索，给出有来源引用的答案。

### 技术前瞻：从 API 调用到语义理解基础设施

早期版本最大的技术局限性在于它本质上只有一个 LLM 调用节点：用户点击"生成摘要"，系统把对话文本发给模型，返回结果渲染为卡片。提示词工程做得再精良，也不能掩盖这一结构上的单薄。

本轮迭代在技术层面做了两个方向上的根本性投入。第一是**向量知识库的实践落地**。每一条捕获的对话，其标题与核心内容会被异步转化为 768 维语义向量，持久化存储在 IndexedDB 的 `vectors` 表中。检索时通过纯 JavaScript 实现的余弦相似度计算完成本地 Top-K 匹配，全程在浏览器端运行，与 Local-First 的设计哲学完全一致。这不仅是"增加了一个搜索功能"——向量化基础设施使得系统第一次具备了理解语义而非匹配字符的能力，也使得 RAG（检索增强生成）在个人知识场景下获得了具体的实践形态：生成摘要时，系统能够自动检索相关历史上下文并注入 Prompt，让摘要成为基于个人知识积累的关联性分析，而不再是孤立文本的压缩。

第二是**轻量级 Multi-Agent 决策链的引入**。我们将原来的单步摘要生成流程，重构为由 Gardener 智能体承载的多步决策链：对话类型分类 → 动态策略与工具集选择 → 标签提取与主题匹配 → 自我验证。每一步的中间结果通过 `AgentStep` 数据结构持久化，可在 AgentThinkingPanel 中完整呈现，供用户追溯系统的决策逻辑。分类阶段采用规则引擎与轻量模型的双轨策略，对特征明显的对话直接给出高置信度结果并跳过 API 调用，对特征模糊的对话才触发模型介入，在准确性与延迟之间寻找实际均衡。

### 工具整合：鲁棒性与决策链的工程化

Gardener Agent 的工具调用逻辑并非顺序执行的简单流水线。Explore 的混合检索策略在关键词召回基础上对向量相似度结果进行加权融合，关键词命中的结果获得 1.2 倍的分数提升，使系统能够同时适应精确查询与语义漫游两种截然不同的使用意图。

在整个 Agent 执行链的容错设计上，我们延续了 v1.8.x 工程收敛阶段确立的 Fallback Hierarchy 原则：任意一步的失败不会导致整个流程崩塌，而是以降级模式继续执行并标注不确定性来源。这与 P0 Summary Runtime Hardening 中建立的"结构化降级优先于文本坍塌"原则一脉相承——系统在最坏情况下仍然会给出结构化的、可解释的输出，而不是静默失败或返回无意义的纯文本。

### 用户体验：当界面成为认知的镜子

心迹在 UI 层面的核心设计命题始终是：如何让一个隐性的、非线性的思维过程变得可见、可触、可回溯？

早期版本的回答是时间轴——这是最直觉的线性叙事方式，但它天然地隐藏了对话之间的横向关联。Network Tab 以 ECharts 力导向图将向量相似度矩阵可视化为动态的思维网络，边的粗细与透明度直接反映语义相似度的强弱。用户第一次可以直观地看到：原来在这三个月里，有一条始终未中断的关于系统设计的思维主线，它跨越了不同平台、不同时期的七次对话。这不是装饰性的可视化，这是产品信念的具象化：意义来自关系，而不来自单个节点。

Explore 的每条回答底部展示来源对话引用卡片，可一键跳转回原始上下文。在大语言模型幻觉问题尚未被根本解决的时代，这种可追溯的引用设计不是加分项，而是诚实性的基础——系统的每一个结论都可以被追溯到你自己说过的话。

### 一个仍然诚实的边界声明

向量知识库目前运行在纯浏览器环境中，对数百条对话规模的个人使用场景，性能是可接受的。但当历史对话量增长到数千条量级，全局相似度矩阵的计算开销将成为必须直面的工程挑战。我们已为此预留了 Web Worker 异步化与增量计算的扩展路径，但这不是当前版本的已实现能力。Gardener Agent 的类型分类在话题模糊或高度个人化的对话上置信度会有所下降，系统目前的处理方式是标注低置信度结果并允许用户手动修正——这是诚实的降级，而非静默的失败。

心迹的演进路径，从来不是从"记录"到"理解"的线性升级，而是一个不断诚实地面对自身局限、然后在局限中寻找可行扩展点的过程。这一轮迭代的意义不在于我们做了多少，而在于产品的技术深度第一次开始追上产品的哲学深度——尽管只是部分地。
