...
import {
  SearchLineIcon,
  SearchSlashIcon
} from "../components/ThreadSearchIcons"
...
function buildConversationCopyText(
  ...
  lines.push(
    `Started At: ${toLocalDateTime(getConversationOriginAt(conversation))}`
  )
...
export function ConversationList({
  ...
  const queryCacheRef = useRef<
    Map<string, Record<number, ConversationMatchSummary>>
  >(new Map())
  ...
  const remoteDateRange = useMemo(
    () => buildDateRangeForPreset(datePreset),
    [datePreset]
  )
  const remotePlatforms = useMemo(() => {
    ...
    return Array.from(selectedPlatforms).sort()
  }, [selectedPlatforms])
  const hasListFilters = datePreset !== "all_time" || selectedPlatforms.size > 0
  ...
  }, [conversations, normalizedSearchQuery, resultSummaryMap])
  ...
  const grouped = useMemo(() => {
    ...
    if (week.length > 0)
      groups.push({ label: "Started This Week", items: week })
    if (older.length > 0)
      groups.push({ label: "Started Earlier", items: older })
    return groups
  ...
  if (conversations.length === 0) {
    if (hasListFilters || normalizedSearchQuery.length > 0) {
      return (
          ...
          <div className="flex flex-col gap-2">
            {group.items.map((item) => (
              <ConversationCard
                ...
                messageExcerpt={
                  item.matchedInMessagesOnly
                    ? item.summary?.bestExcerpt ?? null
                    : null
                }
